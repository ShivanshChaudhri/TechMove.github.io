...
<ul class="topnav">
  ...
  <li class="right"><a href="#about">About</a></li>
	...
	<form id="myForm" method="post" action="tmdbUpdate.php" > <!-- "<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>",  autocomplete="on" name="myForm" enctype="text/plain" accept-charset="" oninput="" target="" -->
...
if($input_password == $user_password and $input_user_name == $user_name){
...
?>
	...
	<h1>Sign Up Now!<span>Sign up and tell us what you think of the site!</span></h1>
			...
			<div class="section"><span class="number">1</span>General Information</div>
				...
				<label for="un">E-mail :</label><input type="email" name="email" value = "<?php echo $row['email'];?>" size="" required autocomplete="on" onblur="recheckForm();">
				<br>
				<label for="ws">Website :</label><input type="url" name="website" value = "<?php echo $row['website'];?>" id="website" size="" required autocomplete="on" onblur="recheckForm();">
				<br>
				<label for="gender" >Gender :</label>Male:<input type="radio" name="gender" value="Male" id=""<?php if($row['gender'] == 'Male'){ echo 'checked';}?>   >
												       | Female:<input type="radio" name="gender" value="Female" id="">				
				<br>
				<label for="dob">Date of birth :</label><input type="date" name="dob" id="dob" value = "<?php echo $row['dob'];?>" placeholder="dd/mm/yyyy" required autocomplete="on" onblur="recheckForm();">	
				<br>
			...
			<div class = "inner_wrap">
			<label for="email">Email :</label> <input type="email" id="email" name="email" value = "<?php echo $row['email'];?>" required onblur="recheckForm();" autocomplete ="on">
			<br>
			<label for="contactno">Mobile :</label> <input type="tel" id="contactno" name="mobile" value = "<?php echo $row['mobile'];?>" required onblur="recheckForm();" autocomplete ="on" >
            <br>			
			<label for="address">Address :</label> <textarea id="address" name="address" value = "<?php echo $row['address'];?>" rows="1" cols="10" required onkeyup="adjust_textarea(this)" onblur="recheckForm();" autocomplete ="on"></textarea> 
			<br>
			<label for="method">Method to cantact you :</label>
            <input type="checkbox"  name="method" value="E-mail" <?php if($row['method_to_contact'] == 'E-mail'){ echo 'checked';}?>>Email
            <input type="checkbox" name="method" value="WhatsApp" <?php if($row['method_to_contact'] == 'WhatsApp'){ echo 'checked';}?>>Whatsapp
            <input type="checkbox"  name="method" Value="In-app chat" <?php if($row['method_to_contact'] == 'In-app chat'){ echo 'checked';}?>>In-app chat
		</div>
...
<div class = "inner_wrap">			
			<label for="un">User Name :</label> <input type="text" id="un" name="user_name" value = "<?php echo $row['user_name'];?>" required>
				<div id="message" style="float:right"></div>
			<br>
			<label for="pw1">Password :</label> <input type="password" id="pw1" name="pw1" value = "<?php echo $row['pw1'];?>" required autocomplete="off"  onblur="check_name()">
			<br>
			<label for="pw2">Confirm Password :</label> <input type="password" id="pw2" name="pw2" value = "<?php echo $row['pw1'];?>" required autocomplete="off">
</div>
	<!-- <input type="submit" value="Submit" formaction="" onclick="recheckForm();" formenctype="" method="" formaction="" formtarget="" > -->	
	<span class="privacy-policy">
     <input type="checkbox" name="" required>You agree to our Terms and Policy. 
     </span>
